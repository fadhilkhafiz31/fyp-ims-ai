...
The chatbot is a two-layer setup: Dialogflow ES handles intent parsing first, and if that comes back weak, a Gemini call takes over with the full inventory dumped into its prompt as context. Both hit the same Cloud Function, which also does the product/location matching — the matcher runs the query against store name, SKU, and category and ranks results with `productScore * 1000 + locationScore`, so a strong product match always beats a strong location match (see `function-ai/index.js`).
...
Gemini needs an API key set as a Firebase secret (`firebase functions:secrets:set GEMINI_API_KEY`) — without it the chatbot falls back to Dialogflow-only responses.
